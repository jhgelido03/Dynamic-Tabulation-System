...
<span class="fs-4"><?php echo $r['title'];?></span>
     ...
     <hr>
<span class="fs-5 text-center">Event Categories</span><hr>
    <ul class="nav nav-pills flex-column mb-auto">
    <?php  if (mysqli_num_rows($result) > 0) {
  while ($row = mysqli_fetch_assoc($result)){
          $categoryName = $row['category_title']; // Change 'category_name' to your actual column name
          $categoryId = $row['category_ID']; // Change 'category_id' to your actual column name
    ?>
        <li class="nav-item">
            <a href="judges_tabulate_new.php?category_ID=<?php echo $categoryId ?>&id=<?php echo $eventid ?>" class="nav-link text-white <?php echo ($categoryId == $activeCategoryID) ? 'active' : ''; ?>" aria-current="page">
