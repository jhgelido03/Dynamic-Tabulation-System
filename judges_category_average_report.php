...
$rankQuery = "SELECT DISTINCT candidates.candidate_name, candidates.candidate_ID
              ...
              ORDER BY (
        --     SELECT AVG(critave_score) AS total_ave
        -- FROM critave_scores where critave_scores.candidate_ID = candidates.candidate_ID AND id = '$id' AND category_ID = '$category_ID'
        --           LIMIT 1
        SELECT totat_
              ) DESC";
...
if (mysqli_num_rows($rankResult) > 0) {
    while ($row = mysqli_fetch_assoc($rankResult)) {
        ...
        $pdf->Cell(50, 10, $row['candidate_name'], 1, 0);
        ...
        $scoresQuery = "SELECT AVG(critave_score) AS total_ave
        FROM critave_scores where candidate_ID = '{$row['candidate_ID']}' AND id = '$id' AND category_ID = '$category_ID' ";
        ...
        if (mysqli_num_rows($scoresSql) > 0) {
            ...
            while ($scoreRow = mysqli_fetch_assoc($scoresSql)) {
                $pdf->Cell(30, 10, $scoreRow['total_ave'], 1, 0);
            }
        } else {
        ...
        //     $pdf->Cell(30, 10, $critaveRow['critave_score'], 1, 0);
        // } else {
