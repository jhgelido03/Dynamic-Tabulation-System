...
$sql = "SELECT * from  judges where j_name = '$cname'";
$csql = mysqli_query($conn, $sql);
...
if(mysqli_num_rows($csql)>0)
{
    echo "<script>Swal.fire({
       ...
       document.querySelectorAll('button.swal2-confirm').forEach(a=>a.onclick=function(){window.parent.location='committee_judges_view.php?id=$eid'});
...
{
    $judges = "INSERT into judges(j_name,j_username,j_password,id) VALUES ('$cname','$cuname','$cpass','$eid')";
$sql = mysqli_query($conn ,$judges);
if($sql)
{
    echo "<script>Swal.fire({
    position: 'center',
...
if(isset($_POST["delete"]))
{
    $eid = $_POST["eid"];
    $cid =$_POST["cid"];
    $event = "DELETE from judges where j_ID='$cid'";
...
<a href="committee_judges.php" class="btn btn-primary back">Back</a>
</div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="exampleModalLabel">Judges</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form action="committee_judges_view.php?id=<?php echo $id  ?>" method="post">
...
<label for="juname" class="fw-bold">Judges Username</label>
            <input type="text" class="form-control " name="juname" >
</div>
<div class="col text-start mt-2">
<label for="jpass" class="fw-bold">Judges Password</label>
            <input type="password" class="form-control " name="jpass" >
</div>
...
<div class="col-12 title-dashboard text-center mt-3">  <h2>Judges</h2></div>
...
<table id="example" class="table table-striped" style="width:90%;margin:auto">
        ...
        ?>
        <tr>
        <form action="committee_judges_view.php?id=<?php echo $row["id"]  ?>" method="post">
            <td><?php echo $row['j_name'] ?></td>
            <td><?php echo $row['j_username'] ?></td>
            <td><?php echo $row['j_password'] ?></td>
            <input type="hidden" value="<?php echo $row['j_ID'] ?>" name="cid" id="cid">
            <input type="hidden" value="<?php echo $row['id'] ?>" name="eid" id="eid">
            <td><a href="committee_judges_edit.php?j_ID=<?php echo $row['j_ID']; ?>&id=<?php echo $row["id"]; ?>" class="btn btn-primary"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></a>   
