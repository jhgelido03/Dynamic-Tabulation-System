...
{
 ?>
     <form action="admin_candidates_individual_view_edit.php?id=<?php echo $eid;?>&candidate_ID=<?php echo $cid; ?>" method="post" enctype="multipart/form-data">
...
<label for="cnum" class="fw-bold">Candidate No.</label>
...
<label for="cbday" class="fw-bold">Birthdate</label>
            <input type="date" class="form-control " name="cbday" value="<?php  echo $rows["candidate_bday"];?>" max="<?php echo date('Y-m-d', strtotime('-18 years')); ?>">
</div>

<div class="col text-start mt-2">
<label for="ccontact" class="fw-bold">Candidate Contact No.</label>
            <input type="tel" class="form-control " name="ccontact" pattern="[0-9]{11}" value="<?php  echo $rows["candidate_contact"];     ?>">
...
<div class="col-4 mt-5 text-start">
    <a href="admin_candidates_view.php?id=<?php echo $rows["id"]?>" class="btn btn-danger">Close</a>
</div>
...
//        })
//        document.querySelectorAll('button.swal2-confirm').forEach(a=>a.onclick=function(){window.parent.location='admin_candidates.php'});
...
{
  $event = "UPDATE candidates SET candidate_name='$cname', candidate_no='$cnum',candidate_dept='$cdept',candidate_ys = '$cys', candidate_sex = '$csex', candidate_bday = '$cbday', candidate_contact = '$ccontact' where candidate_ID = '$cid' ";
...
move_uploaded_file($tempname ,$folder.$cimage);
    $event = "UPDATE candidates SET candidate_name='$cname', candidate_dept='$cdept',candidate_ys = '$cys', candidate_sex = '$csex', candidate_bday = '$cbday', candidate_contact = '$ccontact',candidate_image = '$cimage' where candidate_ID = '$cid' ";
$sql = mysqli_query($conn ,$event);
if($sql)
{
    echo "<script>Swal.fire({
