...
include("functions.php");
    ...
    $sql = "SELECT * from  candidates INNER JOIN totalcategory_ave ON candidates.candidate_ID = totalcategory_ave.candidate_ID INNER JOIN schedule_list ON candidates.id = schedule_list.id INNER JOIN event_category ON totalcategory_ave.category_ID = event_category.category_ID where schedule_list.id = '$eventid' AND event_category.category_ID = '$categoryid'  ORDER BY category_overall DESC";
$csql = mysqli_query($conn, $sql);

$event = "SELECT * from event_category where category_ID = '$categoryid'";
$cevent = mysqli_query($conn, $sql);
$categorytitle = mysqli_fetch_assoc($cevent);
...
<a href="judges_result_category.php?id=<?php   echo $eventid;    ?>" class="btn btn-primary back">Back</a>
</div>
    <div class="col-6 text-end p-3 ">
    <form action="admin_view_category_overall_report.php?id=<?php echo $eventid; ?>&category_ID=<?php echo $categoryid;      ?>" method="post">
...
<div class="col-12 title-dashboard text-center mt-3">  <h2>Best in <?php  echo $categorytitle["category_title"];   ?></h2></div>
...
<table id="example" class="table table-striped" style="width:90%;margin:auto">
        ...
        <td><?php echo $i;?></td>
            ...
            <td><?php echo $row['category_overall'] ?>%</td>
