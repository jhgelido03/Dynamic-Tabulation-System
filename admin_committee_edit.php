...
$eid = $_GET["id"];
    ...
    $c_ID = $_GET["c_ID"];
    $sql = "SELECT * from  committee where c_ID = '$c_ID'";
...
{
 ?>
     <form action="admin_committee_edit.php?id=<?php echo $eid   ?>&c_ID=<?php echo $c_ID ?>" method="post">
<div class="col-10 text-start mt-2">
<label for="cname" class="fw-bold">Committee Name</label>
            <input type="text" class="form-control " name="cname" value="<?php  echo $rows["c_name"];    ?>">
</div>
<div class="col-10 text-start mt-2">
<label for="cuname" class="fw-bold">Committee Username</label>
            <input type="text" class="form-control " name="cuname" value="<?php    echo $rows["c_uname"];    ?>">
</div>
<div class="col-10 text-start mt-2">
<label for="cpass" class="fw-bold">Commitee Password</label>
    <input type="password" class="form-control " name="cpass" value="<?php   echo $rows["c_password"];   ?>">
</div>
</div>
...
<div class="col-4 mt-5 text-start">
    <a href="admin_committee_view.php?id=<?php echo $rows['id']; ?>" class="btn btn-danger">Cancel</a>
</div>
<input type="hidden" value="<?php  echo $rows["c_ID"];?>" name="cid">
        </form>
        <?php
}
}
...
$cpass = $_POST["cpass"];
$cid = $_POST["cid"];
$event = "UPDATE committee SET c_name='$cname', c_uname='$cuname',c_password = '$cpass' where c_ID = '$cid' ";
