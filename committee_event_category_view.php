...
$sql = "SELECT * from event_category where category_title = '$cname' AND id = '$id'";
...
if(mysqli_num_rows($selectsql)>0)
{
    echo "<script>Swal.fire({
       ...
       document.querySelectorAll('button.swal2-confirm').forEach(a=>a.onclick=function(){window.parent.location='committee_event_category_view.php?id=$eid'});
     
       </script>";
}
else if(empty($cname) || empty($cuname))
{
    echo "<script>Swal.fire({
        position: 'center',
        iconHtml:'<i class=\"fa-solid fa-circle-xmark\" style=\"color: red ;\"></i>',
...
{
    $event_category = "INSERT into event_category(category_title,category_description,id) VALUES ('$cname','$cuname','$eid')";
$sql = mysqli_query($conn ,$event_category);
...
if(isset($_POST["delete"]))
{
    $eid = $_POST["eid"];
    $cid =$_POST["cid"];
    $event = "DELETE from event_category where category_ID='$cid'";
...
<a href="committee_event_category.php" class="btn btn-primary back">Back</a>
</div>
...
<table id="example" class="table table-striped" style="width:90%;margin:auto">
        ...
        <form action="committee_event_category_view.php?id=<?php echo $row["id"]; ?>" method="post">
            ...
            <td><a href="committee_event_criteria.php?category_ID=<?php echo $row['category_ID']; ?>&id=<?php  echo $row["id"]  ?>" class="btn btn-info">Criteria</a>   
            <a href="committee_event_category_edit.php?category_ID=<?php echo $row['category_ID']; ?>&id=<?php  echo $row["id"]  ?>" class="btn btn-primary"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></a>   
            <?php
        $id = $row['id'];
