...
{
 ?>
     <form action="admin_events_edit.php" method="post">
...
<label for="edesc" class="fw-bold">Number of Candidates</label>
            <input type="text" class="form-control " name="noc" value="<?php echo $rows["no_of_candidates"]; ?>">
</div>
<div class="col text-start mt-2">
<label for="etype" class="fw-bold">Event Type</label>
<select class="form-select" aria-label="Default select example" name="etype" id="csex" disabled>
  <option selected>Choose one from the list</option>
  <option value="0"  <?php if ($rows['event_type'] == "0") echo ' selected="selected"'; ?>>Individual</option>
  <option value="1"  <?php if ($rows['event_type'] == "1") echo ' selected="selected"'; ?>>Group</option>
</select>
</div>
<div class="col text-start mt-2">
...
<div class="col-4 mt-5 text-start">
    <a href="admin_events.php" class="btn btn-danger">Cancel</a>
</div>
...
$eid = $_POST["eid"];
$noc = $_POST["noc"];
$event = "UPDATE schedule_list SET title='$title', description='$desc',no_of_candidates = '$noc',start_datetime = '$sdate', end_datetime = '$edate' where id = '$eid' ";
