...
<a href="committee_candidates.php" class="btn btn-primary back">Back</a>
</div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="committee_candidates_view.php?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
<div class="col text-start">
<label for="gcame" class="fw-bold">Group Name</label>
            <input type="text" class="form-control " name="gcname">
</div>
<div class="col text-start mt-2">
<label for="gcdept" class="fw-bold">Group Department</label>
<select class="form-select" aria-label="Default select example" name="gcdept" id="gcdept">
...
<label for="gccontact" class="fw-bold">Group Contact No.</label>
            <input type="number" class="form-control " name="gccontact">
</div>
<div class="col text-start mt-2">
<label for="gcimage" class="fw-bold">Group Image</label>
            <input type="file" class="form-control " name="gcimage" accept=".jpg,.png,.jpeg">
</div>
...
<select class="form-select" aria-label="Default select example" name="icdept" id="cdept">
  <option selected>Choose one from the list</option>
  <option value="BSBA - Bachelor of Science in Business Administration">BSBA - Bachelor of Science in Business Administration</option>
  <option value="BSIT - Bachelor of Science in Information Technology">BSIT - Bachelor of Science in Information Technology</option>
  <option value="BSOA - Bachelor of Science in Office Administration">BSOA - Bachelor of Science in Office Administration</option>
...
<label for="icys" class="fw-bold">Candidate Year and Section</label>
            <input type="text" class="form-control " name="icys">
</div>
<div class="col text-start mt-2">
<label for="icsex" class="fw-bold">Candidate Sex</label>
<select class="form-select" aria-label="Default select example" name="icsex" id="icsex">
  <option selected>Choose one from the list</option>
...
<label for="icbday" class="fw-bold">Candidate Bday</label>
            <input type="date" class="form-control " name="icbday" max="<?php echo date('Y-m-d', strtotime('-18 years')); ?>">
</div>
<div class="col text-start mt-2">
...
<label for="icimage" class="fw-bold">Candidate Image</label>
            <input type="file" class="form-control " name="icimage" accept=".jpg,.png,.jpeg">
</div>
<input type="hidden" value="<?php   echo $id     ?>" name="eid">
<!-- <div class="col text-start mt-2">
<label for="edate" class="fw-bold">End Date</label>
...
<table id="example" class="table table-striped" style="width:90%;margin:auto">
        ...
        ?>
        <tr>
        <form action="committee_candidates_view.php?id=<?php echo $row["id"]; ?>" method="post">
            ...
            <td><a href="committee_candidates_viewall.php?candidate_ID=<?php echo $row['candidate_ID']; ?>" class="btn btn-info"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></a> 
            <a href="<?php 
    if ($type["event_type"] == '0') {
        echo 'committee_candidates_individual_view_edit.php?candidate_ID=' . $row['candidate_ID'] . '&id=' . $row["id"];
    } elseif ($type["event_type"] == '1') {
        echo 'committee_candidates_group_view_edit.php?candidate_ID=' . $row['candidate_ID'] . '&id=' . $row["id"];
    }
...
$sql = "SELECT * from  candidates where candidate_name = '$gcname' AND candidate_bday='$gcbday'";
$csql = mysqli_query($conn, $sql);
...
else if(empty($gcname) || empty($gcdept) || empty($gcys) || empty($gcsex) || empty($gcbday) || empty($gccontact) || empty($gcimage)  )
{
    echo "<script>Swal.fire({
        position: 'center',
        iconHtml:'<i class=\"fa-solid fa-circle-xmark\" style=\"color: red ;\"></i>',
...
{
  $candidates = "INSERT into candidates(candidate_name,candidate_dept,candidate_ys,candidate_sex,candidate_bday,candidate_contact,candidate_image,id) VALUES ('$gcname','$gcdept','$gcys','$gcsex','$gcbday','$gccontact','$gcimage','$eid')";
...
$sql = "SELECT * from  candidates where candidate_name = '$icname' AND candidate_bday='$icbday'";
$csql = mysqli_query($conn, $sql);
...
else if(empty($icname) || empty($icdept) || empty($icys) || empty($icsex) || empty($icbday) || empty($iccontact) || empty($icimage)  )
{
    echo "<script>Swal.fire({
        position: 'center',
        iconHtml:'<i class=\"fa-solid fa-circle-xmark\" style=\"color: red ;\"></i>',
...
{
  $candidates = "INSERT into candidates(candidate_name,candidate_dept,candidate_ys,candidate_sex,candidate_bday,candidate_contact,candidate_image,id) VALUES ('$icname','$icdept','$icys','$icsex','$icbday','$iccontact','$icimage','$eid')";
$sql = mysqli_query($conn ,$candidates);
if($sql)
{
    echo "<script>Swal.fire({
